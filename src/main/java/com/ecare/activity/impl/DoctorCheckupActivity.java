...
@Component("doctorCheckupActivity")
public class DoctorCheckupActivity implements StepFunctionActivity {
	private static final Logger LOGGER = LoggerFactory.getLogger(DoctorCheckupActivity.class);
	@Autowired
	...
	public void performActivity(Map<String, Object>requestMap) throws Exception {
			...
			GetActivityTaskResult getActivityTaskResult = awsStepFunctionClient.getActivityTask(new GetActivityTaskRequest()
					.withActivityArn(ACTIVITY_DOCTOR_CHEKUP_ARN));
			...
			patientStatusFromDB.get().setPtCurrentState(ActivityEnum.DOCTORCHEKUP.getActivityName());
			patientStatusFromDB.get().setNextActivity(ActivityEnum.REVIEW.getActivityName());
			Map<String,Long> actvityMap = new HashMap<String,Long>();
			actvityMap.put(ActivityEnum.DOCTORCHEKUP.getActivityName(), patientStatus.getPtCurrentDate());
