...
public class ReviewActivity implements Activity {
	...
	public void performActivity(Map<String, Object>requestMap) throws Exception {
			...
			GetActivityTaskResult getActivityTaskResult = awsStepFunctionClient
					.getActivityTask(new GetActivityTaskRequest().withActivityArn(ACTIVITY_REVIEW_ARN));
			...
			patientStatusFromDB.get().setPtCurrentState(ActivityEnum.REVIEW.getActivityName());
			patientStatusFromDB.get().setNextActivity(ActivityEnum.PHARMACY.getActivityName());
			Map<String,Long> actvityMap = new HashMap<String,Long>();
			actvityMap.put(ActivityEnum.REVIEW.getActivityName(), patientStatus.getPtCurrentDate());
